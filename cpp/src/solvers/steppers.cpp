...
void ader_stepper(Vecr u, Vecr ub, Vecr wh, Vecr qh, int ndim, int nx, int ny,
                  ...
                  bool STIFF, int FLUX, bool PERR_FROB, Par &MP) {

  weno_launcher(wh, ub, ndim, nx, ny, nz);

  predictor(qh, wh, ndim, dt, dx, dy, dz, STIFF, false, MP);

  fv_launcher(u, qh, ndim, nx, ny, nz, dt, dx, dy, dz, true, true, FLUX,
              PERR_FROB, MP);
}
...
void split_stepper(Vecr u, Vecr ub, Vecr wh, int ndim, int nx, int ny, int nz,
                   ...
                   bool HALF_STEP, int FLUX, bool PERR_FROB, Par &MP) {
  ...
  fv_launcher(u, wh, ndim, nx, ny, nz, dt, dx, dy, dz, false, false, FLUX,
              PERR_FROB, MP);
