...
int ind(int t, int i, int nx) { return t * nx + i; }

int ind(int t, int i, int j, int nx, int ny) { return (t * nx + i) * ny + j; }
...
void centers2(Vecr u, Vecr rec, int nt, int nx, int ny, double dt, double dx,
              double dy, Vecnr WGHTS_T, bool SOURCES, Par &MP) {
...
void interfs2(Vecr u, Vecr rec, int nt, int nx, int ny, double dt, double dx,
              double dy, Vecnr WGHTS_T, bool PERRON_FROBENIUS, Par &MP) {
  double kx = dt / (2 * dx);
  double ky = dt / (2 * dy);
...
void fv_launcher(Vecr u, Vecr rec, int ndim, int nx, int ny, int nz, double dt,
  ...
  switch (ndim) {
  case 1:
    centers1(u, rec, nt, nx, dt, dx, WGHTS_T, SOURCES, MP);
    interfs1(u, rec, nt, nx, dt, dx, WGHTS_T, PERRON_FROBENIUS, MP);
    break;
  case 2:
    centers2(u, rec, nt, nx, ny, dt, dx, dy, WGHTS_T, SOURCES, MP);
    interfs2(u, rec, nt, nx, ny, dt, dx, dy, WGHTS_T, PERRON_FROBENIUS, MP);
    break;
    // case 3 : TODO
  }
}
