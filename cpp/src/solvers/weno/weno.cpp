...
{   // Calculate coefficients of basis polynomials and weights
    ...
    for (int i=0; i<N1; i++)
        ...
        if (N==1)
            ...
            ret(i,j) = (oL(j)*wL(i,j)+oR(j)*wR(i,j)+oCL(j)*wCL(i,j)) / oSum(j);
        else
            ret(i,j) = (oL(j)*wL(i,j)+oR(j)*wR(i,j)+oCL(j)*wCL(i,j)+oCR(j)*wCR(i,j)) / oSum(j);
        }
}
...
{   // NOTE: boundary conditions extend u by two cells in each dimension
    ...
    switch (ndim)
    {
        case 1 : weno1(ret, u, nx+2, ny, nz);
                 break;
        case 2 : weno2(ret, u, nx+2, ny+2, nz);
                 break;
        case 3 : weno3(ret, u, nx+2, ny+2, nz+2);
                 break;
    }
}
