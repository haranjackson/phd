...
double distanceMarcher::solveQuadratic(int i, const double &a, const double &b,
                                       double &c) {
  c -= 1;
  double r0 = 0;
  double r1 = 0;
  ...
  for (int i = 0; i < size_; i++)
    if (flag_[i] == Far) {
      ...
      if (borders) {
        ...
        for (int dim = 0; dim < dim_; dim++) {
          double dist = fabs(ldistance[dim]);
          if (dist > 0.)
        ...
        /*
          ...
          for (int dim = 0; dim < dim_; dim++)
            if (ldistance[dim] > 0)
              dsum += 1 / ldistance[dim] / ldistance[dim];
          if (phi_[i] < 0)
