...
def first_stokes_problem_IC():
    ...
    QL = Qvec(ρ, p, -v, A, J, 0, PAR)
    QR = Qvec(ρ, p,  v, A, J, 0, PAR)
    u = zeros([nx, ny, nz, 18])
...
def viscous_shock_IC(center=0):
    ...
    for i in range(nx):
        ...
        J = zeros(3)
        λ = 1
        u[i,0,0] = Qvec(ρ[i], p[i], array([v[i], 0, 0]), A, J, λ, PAR)
...
def heat_conduction_IC():
    ...
    QL = Qvec(ρL, p0, v0, AL, J0, 0, PAR)
    QR = Qvec(ρR, p0, v0, AR, J0, 0, PAR)
    u = zeros([nx, ny, nz, 18])
    x0 = Lx / 2
    for i in range(nx):
        if i*dx < x0:
...
def semenov_temp(dataArray, PAR):
    ...
    return [Qvec_to_Pclass(state, PAR, 0, 0, 1).T for state in states]
