...
def s_max(pL, pR, d, params, subsystems):
    max1 = max_abs_eigs(pL, d, params, subsystems)
    max2 = max_abs_eigs(pR, d, params, subsystems)
    return max(max1, max2) * (pR - pL)

def Drus(pL, pR, d, pos, params, subsystems):
    """ Returns the Rusanov jump term at the dth boundary
    """
    if pos:
        ret = flux(pR, d, params, subsystems)
        ret += flux(pL, d, params, subsystems)
        ret += Bint(pL, pR, d, subsystems.viscous)
    else:
        ret = -flux(pR, d, params, subsystems)
        ret -= flux(pL, d, params, subsystems)
        ret -= Bint(pL, pR, d, subsystems.viscous)
    ret -= s_max(pL, pR, d, params, subsystems)
    return ret
